...
router.setState = function (toPage) {
   ...
   *    4. For each <journal-entry> element, you can grab the JSON version of its info with .entry 
   *      (e.g. someJournalEntryElement.entry)
   * 
   *       a. This is useful when viewing a single entry. You may notice an <entry-page> element in the HTML, 
   *          this is the element that is displayed when the .single-entry class is applied to the body.
   *          You can populate this element by using .entry similarly. So if I wanted to grab a specific 
   *          <journal-entry> and populate it's info into the <entry-page>, I would simply use an assignment of 
   *          entryPageElement.entry = journalEntryElement.entry
   * 
   *       b. Clearing the <entry-page> element of its previous data can be a bit tricky, it might be useful to 
   *          just delete it and insert a new blank one in the same spot each time. Just a thought.
   *
